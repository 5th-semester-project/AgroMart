...
export async function GET(req, { params }) {
    try {
        ...
        const customDateRange = url.searchParams.get('customDateRange');

        let startDate, endDate;
        const currentDate = new Date();

        if (duration === 'CustomRange' && customDateRange) {
            const parsedCustomDateRange = JSON.parse(customDateRange);
            startDate = new Date(parsedCustomDateRange.from);
            endDate = new Date(parsedCustomDateRange.to);
        } else {
            if (duration === 'weekly') {
                startDate = new Date();
